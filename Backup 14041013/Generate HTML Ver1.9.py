...
DEFAULT_COVER = 'Books Images/default_cover.png'
...
def generate_html():
    ...
    for _, row in df.iterrows():
        ...
        if IRANKETAB_IMAGE and iranketab_filename:
            image_path = f"{IRANKETAB_URL_PREFIX}/{iranketab_filename}"
        else:
            fname = f"{cleaned_isbn}.jpg" if cleaned_isbn else DEFAULT_COVER
            image_path = f"{IMAGE_BASE_PATH_LOCAL}/{fname}"
...
/* --- HEADER STYLES 3.0 (Hybrid Layout) --- */
.header {
    display: flex;
...
/* Desktop Layout: Title Right | Controls Left */
@media(min-width: 950px) {
    ...
    .header h1 {
        white-space: nowrap;
        margin-top: 8px; /* تراز بصری با اینپوت جستجو */
    }

    .left-column {
        flex-grow: 1;
        max-width: 1920px; /* جلوگیری از کش آمدن بیش از حد */
        display: flex;
        flex-direction: column;
        gap: 10px;
    }
    ...
    /* جستجو تمام فضای خالی بین عنوان و دکمه‌ها را پر کند */
    input[type="search"] {
        flex-grow: 1; 
    }
    
    .buttons-group {
        flex-shrink: 0; /* دکمه‌ها جمع نشوند */
        display: flex;
        gap: 5px;
    }
}
...
.btn {
    font-family: inherit; background: #fff; border: 1px solid #e0e0e0; padding: 8px 15px;
    border-radius: 8px; cursor: pointer; transition: all 0.2s; color: #555;
    font-size: 0.9rem; white-space: nowrap;
}
.btn:hover { background: #f9f9f9; }
.btn.active { background: #D00400; color: #fff; border-color: #D00400; }

/* Stats Bar (Under Controls) */
.stats-bar {
    font-size: 0.85rem;
    color: #777;
    font-weight: 500;
    padding-right: 5px;
    text-align: left; /* در جهت RTL یعنی سمت چپ صفحه */
    padding-left: 5px;
}
.stats-highlight {
    color: #D00400;
...
<div class="header">
    ...
    <!-- سمت چپ: ستون کنترل‌ها -->
    <div class="left-column">
        <!-- ردیف 1: جستجو + دکمه‌ها -->
...
'''
    ...
    final_html = final_html.replace('__DEFAULT_COVER__', DEFAULT_COVER)
